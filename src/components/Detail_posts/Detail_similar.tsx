...
export default function DetailSimilar({products}: ProductProps){
  ...
  return(
    <div className="w-[600px] mt-15 text-center">
      ...
      <div className="mt-8">
        <Swiper slidesPerView="auto" spaceBetween={12} className="w-[600px] overflow-hidden h-40 gap-4 mb-6 bg-gray-300 rounded-xl">
          {products.map((product) => (
            <SwiperSlide key={product._id} className="group relative !w-40 !h-40 rounded-2xl border overflow-hidden">
              <Link href={`/products/${product._id}`}>
                  ...
                  <div className="w-full absolute bottom-2">
                    <span className="w-full bg-opacity-80 text-center text-size-sm font-light">{product.name}</span>
